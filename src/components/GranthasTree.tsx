...
function TreeNode({ node, depth, expanded, searchActive, selectedPath, onToggle, onSelect }: TreeNodeProps) {
    ...
    return (
        <div className="granthas-tree-node">
            <button
                type="button"
                className={`granthas-tree-row ${isSelected ? 'is-selected' : ''}`}
            ...
            >
                <span className={`granthas-tree-caret ${isFolder ? '' : 'empty'}`} aria-hidden="true">
                    {isFolder ? (isExpanded ? '▾' : '▸') : ''}
                </span>
            ...
            {isFolder && isExpanded && node.children.length > 0 && (
                <div>
                    {node.children.map((child) => (
                        <TreeNode key={child.id} node={child} depth={depth + 1} expanded={expanded} searchActive={searchActive} selectedPath={selectedPath} onToggle={onToggle} onSelect={onSelect} />
                    ))}
                </div>
            )}
        </div>
...
export default function GranthasTree({ paths, selectedPath, onSelect }: GranthasTreeProps) {
    ...
    const [expanded, setExpanded] = useState<Set<string>>(() => {
        ...
        for (const node of tree) if (node.type === 'folder') initial.add(node.id);
        return initial;
    });
        ...
        <aside className="granthas-tree-panel" aria-label="Granthas folder tree">
            ...
            <div className="granthas-tree-scroll" role="tree">
                {visibleTree.length > 0 ? visibleTree.map((node) => (
                    <TreeNode key={node.id} node={node} depth={0} expanded={visibleExpanded} searchActive={searchActive} selectedPath={selectedPath} onToggle={toggle} onSelect={onSelect} />
                )) : <div className="granthas-tree-empty">No Granthas found.</div>}
            </div>
        </aside>
    );
}
