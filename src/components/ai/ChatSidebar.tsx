...
function ChatRow({ conversation, active, onClick, onRename, onTogglePin, onTogglePinHover, onDeleteRequest }: {
    ...
    return (
        <div className={`chat-row ${active ? 'chat-row-active' : ''}`}>
            {editing ? (
                <input className="chat-row-rename-input" value={draft} autoFocus onClick={(e) => e.stopPropagation()} onChange={(e) => setDraft(e.target.value)} onBlur={commitRename} onKeyDown={(e) => { if (e.key === 'Enter') commitRename(); if (e.key === 'Escape') setEditing(false); }} />
            ) : (
            ...
            {!editing && (
                <>
                    <button type="button" className="chat-row-pin" aria-label={conversation.pinned ? 'Unpin conversation' : 'Pin conversation'} onClick={(e) => { e.stopPropagation(); onTogglePin(); }}>
                    ...
                    <ChatContextMenu pinned={conversation.pinned} onRename={() => { setDraft(conversation.title); setEditing(true); }} onTogglePin={onTogglePinHover} onDelete={onDeleteRequest} />
                </>
            )}
        </div>
    );
...
export default function ChatSidebar({ conversations, activeConversationId, mobileOpen, onCloseMobile, onOpenMobile, onNewChat, onSelect, onRename, onTogglePin, onDelete, onClearAll }: ChatSidebarProps) {
    ...
    const [desktopOpen, setDesktopOpen] = useState(() => typeof window !== 'undefined' && window.matchMedia('(min-width: 901px)').matches);
    const trimmed = search.trim().toLowerCase();

    const filtered = useMemo(() => {
        if (!trimmed) return conversations;
        return conversations.filter((c) => c.title.toLowerCase().includes(trimmed) || c.messages.some((m) => m.content.toLowerCase().includes(trimmed)));
    ...
    return (
        <>
            ...
            <aside className={`chat-sidebar ${desktopOpen || mobileOpen ? 'chat-sidebar-open' : ''}`} aria-label="Chat history">
                <div className="chat-sidebar-header">
                    <div className="chat-sidebar-header-actions">
                        ...
                        <button type="button" className="chat-sidebar-close-btn" onClick={handleCloseSidebar} aria-label="Close chat sidebar" title="Close chat sidebar">
                ...
                <div className="chat-sidebar-scroll">
                    {pinned.length > 0 && <div className="chat-section"><div className="chat-section-title">Pinned</div>{pinned.map((c) => <ChatRow key={c.id} conversation={c} active={activeConversationId === c.id} onClick={() => handleSelect(c.id)} onRename={(title) => onRename(c.id, title)} onTogglePin={() => onTogglePin(c.id)} onTogglePinHover={() => onTogglePin(c.id)} onDeleteRequest={() => requestDelete(c)} />)}</div>}
                    <div className="chat-section">
                        ...
                        {recentsSorted.map((c) => <ChatRow key={c.id} conversation={c} active={activeConversationId === c.id} onClick={() => handleSelect(c.id)} onRename={(title) => onRename(c.id, title)} onTogglePin={() => onTogglePin(c.id)} onTogglePinHover={() => onTogglePin(c.id)} onDeleteRequest={() => requestDelete(c)} />)}
                    </div>
                </div>
            ...
            {pendingDelete && <div className="chat-confirm-overlay" onClick={() => setPendingDelete(null)}><div className="chat-confirm-dialog" onClick={(e) => e.stopPropagation()}><h3>Delete conversation?</h3><p>"{pendingDelete.title}" will be permanently deleted. This action cannot be undone.</p><div className="chat-confirm-actions"><button type="button" className="chat-confirm-cancel" onClick={() => setPendingDelete(null)}>Cancel</button><button type="button" className="chat-confirm-danger" onClick={confirmDelete}>Delete</button></div></div></div>}
            {confirmClear && <div className="chat-confirm-overlay" onClick={() => setConfirmClear(false)}><div className="chat-confirm-dialog" onClick={(e) => e.stopPropagation()}><h3>Clear all conversations?</h3><p>This will permanently delete all saved conversations. This action cannot be undone.</p><div className="chat-confirm-actions"><button type="button" className="chat-confirm-cancel" onClick={() => setConfirmClear(false)}>Cancel</button><button type="button" className="chat-confirm-danger" onClick={() => { onClearAll(); setConfirmClear(false); handleCloseSidebar(); }}>Clear</button></div></div></div>}
        </>
    );
}
