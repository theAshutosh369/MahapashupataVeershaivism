...
export default function AnswerPanel({ answer, sources, loading, onCopyAnswer }: AnswerPanelProps) {
    ...
    const evidenceItems = useMemo(() => answer.split(/\n\s*\n/).map((part) => part.replace(/\[[0-9]+\]/g, '').trim()).filter((part) => part.length >= 25).map((statement) => ({ statement, source: bestEvidence(statement, sources) })), [answer, sources]);
    ...
    const markdownComponents: Components = {
        ...
        a: ({ href, children }) => href?.startsWith('#cite-') ? <button type="button" className="citation-link" onClick={() => { const index = Number(href.slice(6)); if (sources[index - 1]) openSource(sources[index - 1]); }} aria-label="Open exact source location">{children}</button> : <a href={href} target="_blank" rel="noopener noreferrer">{children}</a>
    };
