...
export function attachRagRoutes(app, { publicRoot }) {
    ...
    app.get('/api/rag/status', async (_req, res) => {
        try {
            ...
            res.json({
                ...
                embeddingModel: getEmbeddingModelName(), embeddingDimension: getEmbeddingDimension(),
                llmProvider: llm.primaryProvider || 'none', llmModel: llm.primaryModel,
                llmMode: llm.mode, llmChain: llm.chain, embeddingStorage: 'Float32 binary',
        ...
        try {
                ...
                async function scanDir(dir) {
                    ...
                    for (const entry of entries) {
                        ...
                        if (entry.isDirectory()) await scanDir(full);
                            ...
                            if (lower.endsWith('.json') || lower.endsWith('.pdf') || lower.endsWith('.txt')) found.push(pathMod.relative(dataRoot, full).split(pathMod.sep).join('/'));
                        }
                    }
                }
                await scanDir(dataRoot);
                const fresh = found.filter(Boolean).sort();
                if (fresh.length > 0) return res.json({ ok: true, datasets: fresh });
            ...
            try {
                ...
                async function scanDirFor(base, dir, ext) {
                    ...
                    for (const entry of entries) {
                        const full = pathMod.join(dir, entry.name);
                        if (entry.isDirectory()) await scanDirFor(base, full, ext);
                        else if (entry.isFile() && entry.name.toLowerCase().endsWith(ext)) datasets.push(pathMod.relative(base, full).split(pathMod.sep).join('/'));
                    }
                }
    ...
    app.post('/api/rag/query/stream', async (req, res) => {
        ...
        try {
            await ensureIndex(dataRoot);
            const {
                query: queryText, selectedDataset = '__ALL__', selectedDatasets, topK = 10,
                answerMode = 'detailed', includeConversationMemory = false, conversationHistory = []
            } = req.body ?? {};
            if (!queryText || typeof queryText !== 'string') return res.status(400).json({ ok: false, error: 'Query text is required.' });
            const trimmedQuery = queryText.trim();
            if (trimmedQuery.length === 0) return res.status(400).json({ ok: false, error: 'Query text cannot be empty.' });

            const datasetSelection = Array.isArray(selectedDatasets) && selectedDatasets.length > 0
                ? selectedDatasets.filter(p => p && typeof p === 'string').map(p => p.trim()) : null;

            res.writeHead(200, { 'Content-Type': 'text/event-stream; charset=utf-8', 'Cache-Control': 'no-cache, no-transform', Connection: 'keep-alive' });
            const send = (event, data) => {
                try { if (event) res.write('event: ' + event + '\n'); res.write('data: ' + JSON.stringify(data) + '\n\n'); } catch { /* client disconnected */ }
            };
            ...
            const execution = await withRequestLogs(requestLogId, async () => {
                ...
                return queryStream(
                    trimmedQuery, selectedDataset, Math.min(25, Number(topK) || 10), answerMode,
                    includeConversationMemory, conversationHistory,
                    { onToken: (token) => { if (!token) return; fullAnswer += token; send('token', token); }, signal: controller.signal },
                    datasetSelection
                );
            });
            result = execution.result;
            capturedLogs = getRequestLogs(execution.state);
            ...
            send('done', {
                ok: true, answer: (result?.answer || fullAnswer || '').trim(), sources: result?.sources || [],
                confidence: result?.confidence || 0, retrievedChunks: result?.retrievedChunks || [], prompt: result?.prompt || '',
                requestLogId, logs: capturedLogs
            });
        } catch (error) {
            ...
            try {
                ...
                send('done', {
                    ok: false, answer: '', sources: [], confidence: 0, retrievedChunks: [], prompt: '', error: error.message,
                    requestLogId, logs: capturedLogs
                });
            } catch { /* ignore */ }
    ...
    app.post('/api/rag/query', async (req, res) => {
        try {
            ...
            if (!queryText || typeof queryText !== 'string') return res.status(400).json({ ok: false, error: 'Query text is required.' });
            const trimmedQuery = queryText.trim();
            if (!trimmedQuery.length) return res.status(400).json({ ok: false, error: 'Query text cannot be empty.' });
    ...
    app.post('/api/rag/clear-cache', async (_req, res) => {
        try { clearEmbeddingCache(); res.json({ ok: true, message: 'Embedding cache cleared' }); }
        catch (error) { res.status(500).json({ ok: false, error: String(error) }); }
    });
    ...
    app.post('/api/rag/index/upload', async (_req, res) => {
        try {
            ...
            if (result.ok) return res.json({ ok: true, message: 'Index uploaded to Supabase Storage', bucket: result.bucket, prefix: result.prefix || '(root)', details: result.results });
            const statusCode = result.reason === 'supabase_disabled' ? 400 : 500;
            res.status(statusCode).json({ ok: false, error: result.reason || 'Upload failed', details: result });
        } catch (error) { res.status(500).json({ ok: false, error: String(error) }); }
    });

    if (process.env.RAG_WARM_INDEX === '1') {
        console.log('[RAG Routes] Warming index on startup...');
        ensureIndex(dataRoot).then(() => console.log('[RAG Routes] Index warmed: ' + (getCurrentIndex()?.chunks?.length || 0) + ' chunks')).catch(error => console.warn('[RAG Routes] Index warmup failed:', error.message));
