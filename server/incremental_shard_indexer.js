...
async function embedChunks(chunks) {
    ...
    for (let start = 0; start < chunks.length; start += BATCH_SIZE) {
        ...
        console.log(`[IncrementalRAG] Embedding batch ${Math.floor(start / BATCH_SIZE) + 1}/${Math.ceil(chunks.length / BATCH_SIZE)} (${texts.length} chunks)`);
        try {
    ...
    async function walk(dir, relative = '') {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                // Legacy shards have no sourceFiles. Existing chunk metadata is
                // authoritative in that case; do not duplicate prebuilt data.
                if (!sourceEntry && isIndexedDataset(idx, normalized)) continue;
                results.push(await appendNewFile({ dataRoot, ragRoot, relPath: normalized }));
            } catch (error) { results.push({ status: 'error', path: normalize(rel), error: error.message }); }
        }
    }
    console.log('[IncrementalRAG] One-time startup scan: ' + dataRoot);
    await walk(dataRoot);
    console.log(`[IncrementalRAG] Startup scan complete: ${results.length} source change(s) processed.`);
    return results;
}

// Development indexing is intentionally startup-only. The previous watcher
// made every file save trigger embedding work and could contend with live RAG
// queries. The server performs exactly one reconciliation when it starts.
export function startIncrementalWatcher() {
    console.log('[IncrementalRAG] Continuous file watching disabled; startup scan only.');
    return () => {};
}
